...
export class OnboardingStep11Component implements OnInit {
  ...
  ngOnInit(): void {
    this.onboardingService
      .getOnboardingData()
      .subscribe((stepData: OnboardingStep[]) => {
        const userWeightStep = stepData.find((d) => d.step === StepName.WEIGHT);

        const userGoalWeightStep = stepData.find(
          (d) => d.step === StepName.DREAM_WEIGHT
        );
        ...
        const data = Array.from({ length: 5 }, (_, weekIndex) => {
          if (weekIndex === 0)
            graphValues.push({ x: categories[0], y: INITIAL_WEIGHT });
          ...
          graphValues.push({
            x: categories[endIndex - 1],
            y: parseFloat(weeklyWeight),
          });
  ...
  onSubmitStep(): void {
    const data = { step: StepName.JOURNEY, data: null };
